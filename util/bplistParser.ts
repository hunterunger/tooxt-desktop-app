...
export const parseBuffer = function (buffer: Buffer) {
    ...
    function parseObject(tableOffset: number): any {
        ...
        function bufferToHexString(buffer: string | any[] | Buffer) {
            let str = "";
            let i;
        ...
        function parseData() {
            ...
            if (objInfo == 0xf) {
                ...
                if (intType != 0x1) {
                    console.error(
                        "0x4: UNEXPECTED LENGTH-INT TYPE! " + intType
                    );
                }
        ...
        function parsePlistString(isUtf16?: number) {
            isUtf16 = isUtf16 || 0;
        ...
        function parseArray() {
            ...
            if (objInfo == 0xf) {
                ...
                if (intType != 0x1) {
                    console.error(
                        "0xa: UNEXPECTED LENGTH-INT TYPE! " + intType
                    );
                }
                const intInfo = int_type & 0x0f;
                const intLength = Math.pow(2, intInfo);
        ...
        function parseDictionary() {
            ...
            if (objInfo == 0xf) {
                ...
                if (intType != 0x1) {
                    console.error(
                        "0xD: UNEXPECTED LENGTH-INT TYPE! " + intType
                    );
                }
                const intInfo = int_type & 0x0f;
                const intLength = Math.pow(2, intInfo);
...
function readUInt64BE(buffer: any, start: any) {
